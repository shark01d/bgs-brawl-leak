...
class ClientThread(Thread):
    ...
    def recvall(self, length: int):
        ...
        while len(data) < length:
            ...
            if not s:
                _(f"{Helpers.red}[*] while receiving data!")
                break
            ...
            while True:
                ...
                if len(header) > 0:
                    ...
                    if packet_id in packets:
                        ...
                        _(f'{Helpers.blue}[*] PacketID: {packet_id}, Name: {packet_name} Length: {packet_length}')
                        ...
                        _(f'{Helpers.green}[*] Unhandled Packet! ID: {packet_id}, Length: {packet_length}')

                if time.time() - last_packet > 10:
                    _(f"{Helpers.green}[*] Client disconnected! IP: {self.address[0]}")
                    self.client.close()
                    self.db.close()
                    _(f"{Helpers.green}[*] Client disconnected! IP: {self.address[0]}")
