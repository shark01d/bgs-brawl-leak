...
class LogicDailyData:
    ...
    def encode(self):
        ...
        self.writeVInt(5184000)  # Trophy Road Timer
        self.writeVInt(86400)      # Power Play Timer
        ...
        self.writeVInt(1)  # Brawl Pass Boolean
        for x in range(1):
            self.writeVInt(3) # Brawl Pass Season(0-Taras Bazzar,1-Summer Of Monsters,2-Starr Park,3-Holiday Getaway)
            self.writeVInt(34500) # Brawl Pass Tokens
            self.writeUInt8(1) # Brawl Pass Purchased
            self.writeVInt(0)
            self.writeUInt8(0)
