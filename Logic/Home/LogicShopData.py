...
class LogicShopData:
    ...
    shop_resources = json.loads(open('Logic/Json/Shop.json', 'r').read())
    ...
    def encodeShopOffers(self):
        ...
        for x in LogicShopData.offers:
            ...
            self.writeUInt8(x['OriginalCost'])
            self.writeVInt(1)

            self.writeInt(0)
            self.writeStringReference(x['OfferName'])

            self.writeUInt8(0)
            self.writeString(x['Background'])
            self.writeVInt(0)
            self.writeUInt8(0)
            self.writeVInt(2)
            self.writeVInt(0)
